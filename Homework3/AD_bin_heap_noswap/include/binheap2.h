...
typedef struct {
    ...
    unsigned int max_size; // maximum number of nodes (size of A)
    size_t key_size; // size of the key type
    total_order_type leq; // heap total order
    void *max_order_value; // maximum value stored in the heap up to now 
    unsigned int *key_pos; // array containing the position of the key of a node
    unsigned int *rev_pos; // array containing the node corresponding to a given position
} binheap_type;
